"""ETL for loading NPAtlas compound data, references, and annotations into BioNexus."""
...
class NPAtlasCompoundJob:
    """
    Single NPAtlas compound job parsed from NPAtlas JSON data.
    """
    ...
    # Unique identifier; database reference
    npaid: str
...
def iter_jobs(jobs: list[NPAtlasCompoundJob]) -> Generator[dict[str, str], None, None]:
    """
    Iterate over NPAtlas compound jobs and yield data for database loading.
    """
    for j in jobs:
        yield {
            "smiles": j.smiles,
...
def refs_for_job(job: NPAtlasCompoundJob) -> list[dict]:
    """
    Generate reference rows for a given NPAtlas compound job.
    """
    db = "NPAtlas"
    dbid = job.npaid
    out: list[dict] = []
...
def annotations_for_job(job: NPAtlasCompoundJob) -> list[dict]:
    out: list[dict] = []
    ...
    # Taxonomy strings
    if job.taxonomy_domain:
        ann("taxonomy", "domain", job.taxonomy_domain)
...
def load_compounds_npatlas(
    ...
    - Inserts:
        * Compound rows
        * Reference rows (NPAtlas npaid + name + synonyms) + compound_reference links
    ...
    # ----------------------------
    # Download + load NPAtlas JSON
    # ----------------------------
    npatlas_path = download_and_prepare(url=NPATLAS_URL, cache_dir=workdir)
    ...
    job_by_id: dict[str, NPAtlasCompoundJob] = {}

    for item in tqdm(data, desc="Parsing NPAtlas JSON"):
        origin = item.get("origin_organism") or {}
        genus = origin.get("genus")
        species = origin.get("species")

        job = NPAtlasCompoundJob(
            inchikey=item.get("inchikey"),
            smiles=item.get("smiles"),
            npaid=item.get("npaid"),
            name=item.get("original_name"),
            synonyms=item.get("synonyms", []),
            taxonomy_domain=origin.get("type"),
            taxonomy_genus=genus,
            taxonomy_species=f"{genus} {species}" if genus and species else None,
        )

        if not job.inchikey or not job.smiles or not job.npaid:
            log.warning("skipping malformed NPAtlas entry (missing inchikey/smiles/npaid)")
            continue

        jobs.append(job)
        job_by_id[job.npaid] = job

    # Filter out jobs that have InChIKeys already in DB
    with SessionLocal() as s:
        existing = set(s.execute(sa.select(Compound.inchikey)).scalars().all())
    
    jobs = [j for j in jobs if j.inchikey not in existing]
    job_by_id = {j.npaid: j for j in jobs}
    log.info(f"Prepared {len(jobs)} NPAtlas compound jobs for processing (after filtering existing).")

    # ----------------------------
    # Progress + counters
    # ----------------------------
    pbar = tqdm(total=len(jobs), desc="Processing NPAtlas compounds")
        ...
        for evt in run_retromol_stream(
            ...
            if evt.error is not None:
                ...
                try:
                    ...
                    npaid = result.submission.props["npaid"]
                    job = job_by_id[npaid]
    ...
    log.info(
        "NPAtlas compound loading completed: "
